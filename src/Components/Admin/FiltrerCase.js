...
function FiltrerCase({func,verife,Data}) {
    ...
    return (
        ...
        <div className="col-lg-12 col-md-12 d-flex flex-column "> 
           ...
           {  Data.map(optio =>{ 
            return(<option key={optio.id} value={verife==="Actualite"?optio.data.name:optio.name}>{verife==="Actualite"?optio.data.name:optio.name}</option>)})}
        </select>
           </div>
           ...
           {  Data.map(optio =>{ 
              let dat
              if(verife=='Actualite'){
              let da=new Date(optio.data.date).getTime()
                 dat=moment(da).format('DD-MM-YYYY')
              }
              return(<option key={optio.id} value={verife==="Actualite"?optio.data.date:optio.Classe}>{verife==="Actualite"?dat:optio.Classe}</option>)})}
        </select>
